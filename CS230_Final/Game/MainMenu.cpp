...
void MainMenu::Update([[maybe_unused]] double dt)
{
	if (Engine::GetInput().KeyJustPressed(CS230::Input::Keys::Down)) {
		...
		{
		case Option::side_scroller:
			current_option = Option::space_shooter;			
			break;
		case Option::space_shooter:
			current_option = Option::project;
			break;
		case Option::project:
...
void MainMenu::Draw()
{
	...
	project_texture->Draw(Math::TranslationMatrix(Math::ivec2{
		...
		Engine::GetWindow().GetSize().y - project_texture->GetSize().y - 400 }));
...
void MainMenu::update_textures()
{
	...
	case Option::side_scroller:
		...
		project_color = non_seleted_color;
	...
	project_texture = Engine::GetFont(static_cast<int>(Fonts::Outlined)).PrintToTexture("Project", project_color);
